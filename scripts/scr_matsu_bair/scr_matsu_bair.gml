	...
	switch(_phase)
		...
		case PHASE.start:
			...
			if (attack_frame==0)
				...
				attack_phase++;
				attack_frame=2;
                var _hitbox = create_melee(-45,-30,1.2,.7,10,10,0.6,12,50,2,HITBOX_SHAPE.rectangle,0,FLIPPER.from_player_center_horizontal);
			...
			if (attack_frame==0)
				...
				attack_phase++;
				attack_frame=5;
				 var _hitbox = create_melee(-42,-30,1,.7,7,7,0.6,12,50,5,HITBOX_SHAPE.rectangle,0,FLIPPER.from_player_center_horizontal);
				set_hitbox_property(_hitbox,HITBOX.snd_hit,snd_bair_hit);
				}
			break;
			}
